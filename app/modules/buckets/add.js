...
module.exports.init = (request_id, data) => {
	return new Promise(async (resolve, reject) => {
		if (data.bucket_name !== 'undefined' && data.cover_image !== 'undefined') {
			resolve();
		} else {
...
module.exports.insertIntoBucketsTable = (request_id, data) => {
	return new Promise(async (resolve, reject) => {
		...
		const { bucket_name, cover_image } = data;
		let type = data.type;
		...
		const query_body = { bucket_id, bucket_name, parent_bucket_id, user_id_added_by, type, cover_image };
