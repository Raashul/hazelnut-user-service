...
logger.info(
	`Database initialized - host: ${config.mysql.provisioning_db_host} database: ${config.mysql
		.provisioning_db_database} max-pool-conn: ${config.mysql.max_conn_limit}.`
);
	...
	pool.getConnection(function(err, connection) {
		if (err) {
			...
			connection.query(sql, function(error, results) {
				connection.release();
				if (error) logger.error(`${name}: Error in connection: ${JSON.stringify(err)}`);
			});
		}
